...
function filterData() {
  ...
  const sourceSheet = ss.getSheetByName("SourceData");

  // --- Setup destination sheets ---
  const filteredName = "Filtered_Active";
  const completedName = "Filtered_Completed";
  ...
  data.forEach(row => {
    if (row[3] === "TargetCategory") { // col D → adjust condition as needed
      if (row[32]) {
...
function createWednesdayTrigger() {
  ...
  ScriptApp.getProjectTriggers().forEach(trigger => {
    if (trigger.getHandlerFunction() === "filterData") {
      ScriptApp.deleteTrigger(trigger);
    }
  });

  // Create a new time-based trigger for every Wednesday at 9am
  ScriptApp.newTrigger("filterData")
    .timeBased()
